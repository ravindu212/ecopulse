...
CHALLENGES = [
    ("Low-Carbon Commute", "low-carbon-commute", "Complete practical lower-carbon travel swaps this week.", ["walk-cycle-short", "public-transport-trip", "combine-errands"]),
    ("Energy Saver", "energy-saver", "Build a simple energy-saving routine at home or campus.", ["switch-off-devices", "reduce-ac", "unplug-idle-electronics"]),
    ("Zero-Waste Starter", "zero-waste-starter", "Take focused first steps toward reducing everyday waste.", ["reusable-bottle", "avoid-single-use", "separate-recycling"]),
]
...
def seed() -> None:
    with SessionLocal() as db:
        ...
        db.commit()
        for title, slug, description, action_slugs in CHALLENGES:
            challenge = db.scalar(select(Challenge).where(Challenge.slug == slug))
            if challenge is None:
                challenge = Challenge(title=title, slug=slug, description=description, active=True)
                db.add(challenge)
                db.flush()
            for sort_order, action_slug in enumerate(action_slugs, start=1):
                action = db.scalar(select(ClimateAction).where(ClimateAction.slug == action_slug))
                if action is None:
                    raise RuntimeError(f"Required climate action is missing: {action_slug}")
                mapping = db.scalar(select(ChallengeAction).where(ChallengeAction.challenge_id == challenge.id, ChallengeAction.action_id == action.id))
                if mapping is None:
                    db.add(ChallengeAction(challenge_id=challenge.id, action_id=action.id, sort_order=sort_order))
        db.commit()
